...
5. **Grant Lakebase permissions to your App's Service Principal**
   ...
   **Option A (recommended) — use the `grant-lakebase-permissions` CLI.** The script resolves the app's service principal, creates the Postgres role, and grants schema + table + sequence privileges for your memory type. All arguments can be passed at runtime; any omitted argument falls back to `.env` or an interactive prompt.
   ...
   | Flag | Description |
   |------|-------------|
   | `--app-name <name>` | Databricks app name. Auto-resolves the SP client ID via `WorkspaceClient.apps.get()`. Use instead of `--sp-client-id`. |
   | `--sp-client-id <uuid>` | Explicit service principal client ID. Alternative to `--app-name`. |
   | `--memory-type <type>` | One of `langgraph-short-term`, `langgraph-long-term`, `openai-short-term`, `long-running-agent`. Determines which schemas/tables get granted. |
   | `--profile <name>` | Databricks config profile (default: `DATABRICKS_CONFIG_PROFILE` from `.env`). |
   | `--instance-name <name>` | Provisioned Lakebase instance name (default: `LAKEBASE_INSTANCE_NAME` from `.env`). |
   | `--project <p> --branch <b>` | Autoscaling Lakebase project + branch (defaults: `LAKEBASE_AUTOSCALING_PROJECT`/`_BRANCH` from `.env`). |

   If some table grants fail on a fresh Lakebase branch because tables don't exist yet, that's expected — send a query to the deployed app to materialize them, then re-run the script.

   **Option B — run the raw SQL.** Useful if you want to grant permissions manually or inspect exactly what's granted. Run the following on your Lakebase instance (replace `app-sp-id` with your app's service principal UUID):
