...
def main():
    ...
    # Prefer --profile, then DATABRICKS_CONFIG_PROFILE from .env (mirrors
    # agent_server.utils.get_user_workspace_client), then default discovery.
    profile = args.profile or os.getenv("DATABRICKS_CONFIG_PROFILE")
    if profile:
        print(f"Using Databricks profile: {profile}", file=sys.stderr)
